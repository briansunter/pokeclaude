...
import { type Card, type TypeStats } from './index.js';
...
export class CardFormatter {
  static formatCards(cards: Card[], format: OutputFormat): string {
    ...
    switch (format) {
      ...
      case 'table':
        return this.formatCardsAsTable(cards);
      case 'compact':
        return this.formatCardsAsCompact(cards);
      default:
        return JSON.stringify(cards, null, 2);
    }
  }
  ...
  static formatCard(card: Card, format: OutputFormat): string {
    switch (format) {
      ...
      case 'table':
        return this.formatSingleCardAsTable(card);
      case 'compact':
        return this.formatSingleCardAsCompact(card);
      default:
        return JSON.stringify(card, null, 2);
    }
  }
  ...
  static formatDeckAnalysis(analysis: DeckAnalysis, format: OutputFormat): string {
    if (format === 'json') {
