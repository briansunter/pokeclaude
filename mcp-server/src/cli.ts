...
import { DuckDBClient, type Card } from './index.js';
import { CardFormatter, type SynergyResult, type DeckAnalysis } from './formatters.js';
...
function separateOutput<T extends { output?: string }>(
  ...
  return { output: output as 'json' | 'table' | 'compact', rest: rest as Omit<T, 'output'> };
}
...
const searchCommand = defineCommand({
  ...
  options: defineOptions(
    z.object({
      ...
      uniqueOnly: z.boolean().optional().describe('Unique cards only (default true)'),
      output: outputOption,
    }),
...
const synergiesCommand = defineCommand({
  ...
  action: async options => {
    ...
    try {
      ...
      console.log(CardFormatter.formatSynergies(result as SynergyResult, output));
    } catch (error) {
...
const analyzeCommand = defineCommand({
  ...
  action: async options => {
    ...
    const cards = await Promise.all(rest.cardNames.map(name => db.getCardByName(name)));
    ...
    if (rest.cardNames.length !== 20) {
      warnings.push(`Deck must be exactly 20 cards (current: ${rest.cardNames.length})`);
    }
    ...
    if (basicCount < 5) {
      warnings.push(`Only ${basicCount} Basic Pokemon detected. Recommend 5-6 minimum`);
    }
    ...
    // Recommendations
    if (trainerCount < 5) {
      recommendations.push('Add more Trainer cards for consistency (aim for 5-8)');
    }
    if (exCount === 0) {
      recommendations.push('Consider adding Pokemon ex for faster wins (2 points per KO)');
    }
    if (basicCount < 5) {
      recommendations.push('Add more Basic Pokemon to ensure consistent starts');
    }
