...
const FALLBACK_SETS: SetInfo[] = [
  ...
  { code: 'A4b', name: 'Deluxe Pack: ex', release_date: '30 Sep 25', total_cards: 379 },
  { code: 'A4a', name: 'Secluded Springs', release_date: '28 Aug 25', total_cards: 105 },
  { code: 'A4', name: 'Wisdom of Sea and Sky', release_date: '30 Jul 25', total_cards: 241 },
  { code: 'A3b', name: 'Eevee Grove', release_date: '26 Jun 25', total_cards: 107 },
  { code: 'A3a', name: 'Extradimensional Crisis', release_date: '29 May 25', total_cards: 103 },
  { code: 'A3', name: 'Celestial Guardians', release_date: '30 Apr 25', total_cards: 239 },
  { code: 'A2b', name: 'Shining Revelry', release_date: '27 Mar 25', total_cards: 111 },
  { code: 'A2a', name: 'Triumphant Light', release_date: '28 Feb 25', total_cards: 96 },
  { code: 'A2', name: 'Space-Time Smackdown', release_date: '29 Jan 25', total_cards: 207 },
  { code: 'A1a', name: 'Mythical Island', release_date: '17 Dec 24', total_cards: 86 },
  { code: 'A1', name: 'Genetic Apex', release_date: '30 Oct 24', total_cards: 286 },
  ...
  try {
    ...
    $('a[href*="/cards/"]').each((_, element) => {
      ...
      if (cardMatch) {
        ...
        const cardName = sanitizeText(img.attr('alt') || img.attr('title') || 'Unknown');
...
async function scrapeCardDetails(card: PokemonCard): Promise<void> {
  try {
    ...
    $('.card-text-attack').each((_, element) => {
      const attackInfo = sanitizeText($(element).find('.card-text-attack-info').text());
      const attackEffect = sanitizeText($(element).find('.card-text-attack-effect').text());
      ...
      if (parts.length >= 15) {
        try {
          ...
          const card = validateCard({
            ...
            abilities: parts[8] ? parts[8].split(';').filter(a => a) : undefined,
            attacks: attacks,
